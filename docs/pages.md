...
| Time slots          | `/cabinet/slots` (+ `?service={serviceId}`) | Schedule across all services: list and create / edit / duplicate / delete slots (startsAt, duration, capacity, price override). Defaults seeded from the service picked in the dialog. `?service=` scopes the list to one service — the services list links here from each card's slot count. |
