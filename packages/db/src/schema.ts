...
export const organizers = pgTable(
  ...
  (t) => [
    ...
    uniqueIndex('organizers_phone_key').on(t.phone),
  ],
)
...
/** Concrete occurrence of a service: start + length; `endsAt` is derived. */
export const timeSlots = pgTable(
  'time_slots',
  {
    ...
    endsAt: timestamp('ends_at', { withTimezone: true }).generatedAlwaysAs(
      sql`starts_at + make_interval(mins => duration_minutes)`,
    ),
...
export const bookings = pgTable(
  'bookings',
  {
    ...
    guestPhone: varchar('guest_phone', { length: 16 }).notNull(),
    messenger: text('messenger').notNull(),
  ...
  (t) => [
    ...
    index('bookings_guest_phone_idx').on(t.guestPhone),
