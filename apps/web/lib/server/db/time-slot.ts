 ...
 * schedule, so past sessions are noise there, but the same function serves any
 ...
 * disappears in between, the foreign key rejects the row, which is the same
...
export async function updateOwnedSlot(
  ...
  // Guard capacity against seats already sold. Read inside the same
  // transaction as the write so a concurrent booking cannot slip between the
 ...
 * bookings is a notification concern for the worker, not a database one.
 */
