 ...
 * and *whether they may write*, then parse the body against a Zod schema.
 * These helpers make the sequence a single expression per handler.
 *
 ...
 *
 * **Scope:** request-level concerns only — sessions, tickets, body parsing.
 * This module must not import from `./db/`: mapping entity failure modes onto
 * status codes belongs to the handler that knows the entity.
 */
 ...
 * The guest counterpart of {@link requireWritableOrganizer}: both answer "who
 * is calling, and may they write" from a credential on the request.
 *
 * The ticket is **consumed**, not peeked: it is a one-shot proof of identity,
 * so a replayed request finds nothing and is refused.
 *
