...
// Next.js) is rewritten here by vercel.json and dispatched by the shared
// mux from pkg/routes. Consolidating 16 per-route functions into one
// keeps the deployment under Vercel Hobby's 12-function limit and lets a
// single warmed instance + connection pool serve the whole API.
//
...
// dispatch — the ServeMux patterns (/api/services/{id}, …) match on the
// real path. Trailing slashes from empty wildcard matches (e.g.
