...
// ParseUpdateOrganizerLanguageInput — the language switcher's payload
// (ADR-011). A single required field validated against the supported
// locale set; the Go API owns this write, the TS server action calls it.
func ParseUpdateOrganizerLanguageInput(body []byte) (string, *Errors) {
	m, e := rawObject(body)
	if e != nil {
		return "", e
	}
	e = NewErrors()
	v, _ := strValue(e, m, "language", true, false, localeRule)
	return v, e.Finish()
}
