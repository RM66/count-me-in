...
// DeleteOwnedSlot — bookings cascade (the time_slots FK). Returns ""
// when nothing matched.
func DeleteOwnedSlot(ctx context.Context, organizerID, slotID string) (string, error) {
	query := fmt.Sprintf(`DELETE FROM time_slots
		WHERE id = $1::uuid AND service_id IN (%s)
		RETURNING id`, ownedServiceIDsAt(2))
	var id string
	err := Pool().QueryRow(ctx, query, slotID, organizerID).Scan(&id)
	if errors.Is(err, pgx.ErrNoRows) {
		return "", nil
	}
	if err != nil {
